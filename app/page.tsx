...
import { Search as SearchIcon, Loader2, Music, Sparkles, ListMusic, Trash2, CheckCircle, Plus, Heart, ChevronRight, ChevronLeft } from 'lucide-react';
...
export default function MelodiaApp() {
  ...
  const [sleepTimer, setSleepTimer] = useState<number | null>(null); // Menit
  ...
  // --- LOGIC SLEEP TIMER (Cycle Mode) ---
  const toggleSleepTimer = () => {
    // Urutan: Off -> 15 -> 30 -> 60 -> Off
    let nextTimer = null;
    if (sleepTimer === null) nextTimer = 15;
    else if (sleepTimer === 15) nextTimer = 30;
    else if (sleepTimer === 30) nextTimer = 60;
    else nextTimer = null;

    if (timerId) clearTimeout(timerId); // Reset timer lama

    if (nextTimer) {
      setSleepTimer(nextTimer);
      showToast(`Musik stop dalam ${nextTimer} menit`);
      const id = setTimeout(() => {
        if (player) player.pauseVideo();
        setIsPlaying(false);
        setSleepTimer(null);
        showToast("Sleep Timer: Musik Stop");
      }, nextTimer * 60 * 1000);
      setTimerId(id);
    } else {
      setSleepTimer(null);
  ...
  const handleCardClick = async (item: any) => {
    if (!item.type || item.type === 'video') { playSong(item); return; }
    setIsLoading(true); setCollectionSongs([]); setCollectionInfo(item);
    const API_KEY = process.env.NEXT_PUBLIC_YOUTUBE_API_KEY;
    try {
      if (item.type === 'playlist') {
        setCurrentView(`yt-playlist-${item.id}`);
        const res = await fetch(`https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=25&playlistId=${item.id}&key=${API_KEY}`);
        const data = await res.json();
        if (data.items) { setCollectionSongs(data.items.map((i:any)=>({id:i.snippet.resourceId.videoId, title:i.snippet.title, artist:i.snippet.videoOwnerChannelTitle||item.artist, cover:i.snippet.thumbnails?.medium?.url||item.cover, type:'video'})).filter((s:any)=>s.title!=='Private video')); }
      } else if (item.type === 'channel') {
        setCurrentView(`artist-${item.id}`);
        const res = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${item.id}&maxResults=20&order=viewCount&type=video&key=${API_KEY}`);
        const data = await res.json();
        if (data.items) { setCollectionSongs(data.items.map((i:any)=>({id:i.id.videoId, title:i.snippet.title, artist:i.snippet.channelTitle, cover:i.snippet.thumbnails?.high?.url, type:'video'}))); }
      }
    } catch (e) { console.error(e); showToast("Gagal memuat"); } finally { setIsLoading(false); }
  };
  ...
  const handleVolumeChange = (e: React.ChangeEvent<HTMLInputElement>) => { const v = parseInt(e.target.value); setVolume(v); if(player) { player.setVolume(v); if(v > 0 && isMuted) { player.unMute(); setIsMuted(false); } } };
  ...
  return (
    <div className="flex flex-col h-screen bg-black text-white overflow-hidden font-sans relative">
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 bg-gradient-to-b from-[#1e1e1e] to-black p-8 overflow-y-auto z-0 pb-32">
          <header className="flex flex-col gap-4 mb-6 pt-4 px-2 sticky top-0 z-40">
          ...
          {currentView === 'search' && ( <SongGrid songs={searchSongs} isLoading={isLoading} currentSong={currentSong} isPlaying={isPlaying} onPlay={handleCardClick} toggleLike={toggleLike} likedSongs={likedSongs} onAddToPlaylistClick={openAddToPlaylistModal} /> )}
          {currentView === 'library' && (
            <div className="max-w-md mx-auto animate-in fade-in slide-in-from-bottom-4">
               ...
               <button onClick={handleCreatePlaylist} className="w-full bg-white/10 hover:bg-white/20 p-4 rounded-lg flex items-center gap-4 mb-6 transition group"><div className="w-12 h-12 bg-gray-700 rounded-md flex items-center justify-center group-hover:bg-white group-hover:text-black transition"><Plus size={24} /></div><div className="text-left flex-1"><h3 className="font-bold">Buat Playlist Baru</h3><p className="text-sm text-gray-400">Tambahkan daftar putar sendiri</p></div></button>
               <div className="space-y-2">
                  <button onClick={() => setCurrentView('liked_songs')} className="w-full bg-[#181818] hover:bg-[#282828] p-4 rounded-lg flex items-center gap-4 transition"><div className="w-12 h-12 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-md flex items-center justify-center"><Heart size={24} fill="white" /></div><div className="text-left flex-1"><h3 className="font-bold text-white">Liked Songs</h3><p className="text-sm text-gray-400">{likedSongs.length} lagu disukai</p></div><ChevronRight className="text-gray-500" /></button>
                  {playlists.map(pl => ( <button key={pl.id} onClick={() => setCurrentView(`playlist-${pl.id}`)} className="w-full bg-[#181818] hover:bg-[#282828] p-4 rounded-lg flex items-center gap-4 transition"><div className="w-12 h-12 bg-gradient-to-br from-cyan-600 to-blue-600 rounded-md flex items-center justify-center"><ListMusic size={24} className="text-white" /></div><div className="text-left flex-1"><h3 className="font-bold text-white">{pl.name}</h3><p className="text-sm text-gray-400">{pl.songs.length} lagu</p></div><ChevronRight className="text-gray-500" /></button> ))}
               </div>
            </div>
          )}
          {currentView === 'liked_songs' && ( <> <div className="relative flex items-end gap-6 mb-8 bg-gradient-to-b from-purple-800 to-transparent p-6 -mx-8 -mt-8 pt-20"><button onClick={() => setCurrentView('library')} className="md:hidden absolute top-4 left-4 bg-black/40 p-2 rounded-full hover:bg-black/60 z-30"><ChevronLeft size={24} color="white" /></button><div className="w-40 h-40 bg-gradient-to-br from-indigo-500 to-purple-500 shadow-2xl flex items-center justify-center rounded-md"><Heart size={64} fill="white" /></div><div><p className="text-sm font-bold mb-2">PLAYLIST</p><h1 className="text-4xl md:text-6xl font-black mb-4">Liked Songs</h1><p className="text-gray-300">{likedSongs.length} lagu</p></div></div><SongGrid songs={likedSongs} isLoading={false} currentSong={currentSong} isPlaying={isPlaying} onPlay={playSong} toggleLike={toggleLike} likedSongs={likedSongs} onAddToPlaylistClick={openAddToPlaylistModal}/> </> )}
          {currentView.startsWith('playlist-') && ( (() => { const plId = currentView.split('-')[1]; const pl = playlists.find(p => p.id === plId); if (!pl) return <div>Playlist tidak ditemukan</div>; return ( <> <div className="relative flex items-end gap-6 mb-8 bg-gradient-to-b from-blue-800 to-transparent p-6 -mx-8 -mt-8 pt-20 group"><button onClick={() => setCurrentView('library')} className="md:hidden absolute top-4 left-4 bg-black/40 p-2 rounded-full hover:bg-black/60 z-30"><ChevronLeft size={24} color="white" /></button><div className="w-40 h-40 bg-gradient-to-br from-cyan-500 to-blue-500 shadow-2xl flex items-center justify-center rounded-md"><ListMusic size={64} className="text-white" /></div><div className="flex-1"><p className="text-sm font-bold mb-2">CUSTOM PLAYLIST</p><h1 className="text-4xl md:text-6xl font-black mb-4">{pl.name}</h1><p className="text-gray-300">{pl.songs.length} lagu</p></div><button onClick={() => handleDeletePlaylist(pl.id)} className="bg-red-500/20 hover:bg-red-500 p-3 rounded-full transition absolute top-20 right-6 md:static"><Trash2 size={24} /></button></div><SongGrid songs={pl.songs} isLoading={false} currentSong={currentSong} isPlaying={isPlaying} onPlay={playSong} toggleLike={toggleLike} likedSongs={likedSongs} onRemoveFromPlaylist={handleRemoveSongFromPlaylist} /> </> ); })() )}
          {(currentView.startsWith('yt-playlist-') || currentView.startsWith('artist-')) && collectionInfo && ( <> <div className="relative flex items-end gap-6 mb-8 bg-gradient-to-b from-red-900 to-transparent p-6 -mx-8 -mt-8 pt-20"><button onClick={() => setCurrentView('search')} className="absolute top-4 left-4 bg-black/40 p-2 rounded-full hover:bg-black/60 z-30"><ChevronLeft size={24} color="white" /></button><div className="w-40 h-40 bg-gray-800 shadow-2xl flex items-center justify-center rounded-md overflow-hidden">{collectionInfo.cover ? <img src={collectionInfo.cover} className="w-full h-full object-cover" /> : <Music size={64} className="text-white" />}</div><div className="flex-1"><p className="text-sm font-bold mb-2 uppercase">{collectionInfo.type === 'channel' ? 'ARTIS' : 'YOUTUBE PLAYLIST'}</p><h1 className="text-3xl md:text-6xl font-black mb-4 line-clamp-2">{collectionInfo.title}</h1><p className="text-gray-300">{collectionInfo.artist}</p></div></div><SongGrid songs={collectionSongs} isLoading={isLoading} currentSong={currentSong} isPlaying={isPlaying} onPlay={handleCardClick} toggleLike={toggleLike} likedSongs={likedSongs} onAddToPlaylistClick={openAddToPlaylistModal} /> </> )}
        </main>
      </div>

      <FullScreenPlayer 
        isExpanded={isPlayerExpanded} onClose={() => setIsPlayerExpanded(false)} currentSong={currentSong} isPlaying={isPlaying} togglePlay={togglePlay} playNext={playNextSong} playPrev={playPrevSong} currentTime={currentTime} duration={duration} handleSeek={handleSeek} toggleLike={toggleLike} isCurrentLiked={isCurrentLiked}
        isShuffle={isShuffle} toggleShuffle={() => setIsShuffle(!isShuffle)}
        repeatMode={repeatMode} toggleRepeat={() => setRepeatMode((prev) => (prev + 1) % 3 as 0 | 1 | 2)}
        sleepTimer={sleepTimer} toggleSleepTimer={toggleSleepTimer} // <-- KIRIM PROPS TIMER BARU
      />

      {currentSong && !isPlayerExpanded && ( 
        <PlayerBar 
          currentSong={currentSong} isPlaying={isPlaying} togglePlay={togglePlay} playNext={playNextSong} playPrev={playPrevSong} currentTime={currentTime} duration={duration} handleSeek={handleSeek} volume={volume} handleVolumeChange={handleVolumeChange} isMuted={isMuted} toggleMute={toggleMute} toggleLike={toggleLike} isCurrentLiked={isCurrentLiked} onExpand={() => setIsPlayerExpanded(true)} 
          isShuffle={isShuffle} toggleShuffle={() => setIsShuffle(!isShuffle)}
          repeatMode={repeatMode} toggleRepeat={() => setRepeatMode((prev) => (prev + 1) % 3 as 0 | 1 | 2)}
          sleepTimer={sleepTimer} toggleSleepTimer={toggleSleepTimer} // <-- KIRIM PROPS TIMER BARU
        /> 
